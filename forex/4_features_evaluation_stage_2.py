...
for currencyPair in currencyPairs:
    ...
    tmp['BuyHoldSell'] = tmp['BuyHoldSell'].shift(-1)
    tmp.dropna(inplace=True)

    totalNumRecords = len(tmp.index)
    buyRecords = len(tmp[tmp['BuyHoldSell'] == 1].index)
    sellRecords = len(tmp[tmp['BuyHoldSell'] == -1].index)
    holdRecords = len(tmp[tmp['BuyHoldSell'] == 0].index)
    ...
    Y_train = train['BuyHoldSell'].values
    ...
    Y_test = test['BuyHoldSell'].values
    ...
    result_rm=cross_val_score(random_forest, train[featuresToBeUsed], train['BuyHoldSell'], cv=10,scoring='accuracy')
    print('The cross validated score for Random Forest Classifier is:',round(result_rm.mean()*100,2))

    #%%
    import matplotlib.pyplot as plt
    import seaborn as sns
    y_pred = cross_val_predict(random_forest, train[featuresToBeUsed], train['BuyHoldSell'], cv=10)
    sns.heatmap(confusion_matrix(train['BuyHoldSell'], y_pred),annot=True,fmt='3.0f',cmap="summer")
